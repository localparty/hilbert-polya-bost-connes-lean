...
Recognized contributions:
  ...
  - Links 1-6 (construction of D_infinity)
-/
...
conditional form (Option-1 DERIVE refactor).**
...
The construction is now `construction_of_gate g` (Bögli + Hurwitz over the
...
theorem rh_via_self_adjointness_of_gate (g : CCMGalerkinSpectralData) : RiemannHypothesis := by
  obtain ⟨H, inst₁, inst₂, inst₃, D, hsa, hspec⟩ := construction_of_gate g
